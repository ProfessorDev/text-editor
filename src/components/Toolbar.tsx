...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  const onYoutubeClick = async () => {
    ...
    if (selectedText.match(urlRegex)) {
      const result = `${beforeText}![youtube](${selectedText})${afterText}`;
      setText(result);
      setSelection(selection.start + 2, selection.start + 2);
    } else {
      const result = `${beforeText}![${selectedText}-youtube](url)${afterText}`;
      if (selectedText === "") {
        setText(result);
        setSelection(selection.start + 2, selection.end + 2);
      } else {
        setText(result);
        setSelection(
          selection.end + 4 + "-youtube".length,
          selection.end + 4 + "url".length + "-youtube".length,
        );
      }
    }
  };
